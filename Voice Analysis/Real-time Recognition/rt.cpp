		...
		: ProcessName(Name)
{
	...
	STARTUPINFO	StartupInfo = {
		...
		(LPSTR)(Name.c_str()) ,	// displayed in title bar for console applications
	...
	BOOL Success = CreateProcess(	NULL,	// application name
					(LPSTR)(Name.c_str()),			// Command line to the process if you want to pass one to main() in the process
	...
	:MutexName(Name)
{
	...
	MutexHandle  = CreateMutex(NULL, bOwned, (LPSTR)(Name.c_str())) ;
...
{																	// bState = SIGNALLED or NOTSIGNALLED to indicate the initial or creation state of the event
	...
	EventHandle = CreateEvent(NULL, bType, bState, (LPSTR)(Name.c_str())) ;		
...
CCondition::CCondition(const string &Name, BOOL bType, BOOL bState) 
{
	...
	ConditionHandle = CreateEvent(NULL, bType, bState, (LPSTR)(Name.c_str())) ;		// based around a Win32 manual event
...
CSemaphore::CSemaphore(const string &Name, int InitialVal, int MaxVal)	// name, starting value and Maximum value needed
	:SemaphoreName(Name)
{
	SemaphoreHandle = CreateSemaphore(0, InitialVal, MaxVal, (LPCSTR)(Name.c_str())) ;
...
CDataPool::CDataPool(const string &Name, UINT size)
	:DataPoolName(Name)
{
	DPInfo.DataPoolHandle = CreateFileMapping(INVALID_HANDLE_VALUE, 
						NULL, 
						PAGE_READWRITE,
						0,
						size,
...
UINT CTimer::SetTimer(UINT Timer, TIMERPROC callbackfn)
{
	...
	TimerID = (UINT)(::SetTimer(NULL, 0, Timer, callbackfn)) ;
...
//	return kbhit() ;
	...
	return _kbhit() ;	// added _ to kbhit for visual C++ 2005 express edition
}
...
{		
	...
	if(!(bSuccess)) {
		...
		MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)buff, 1024, NULL );
		...
		//getch();
		_getch() ;		// change to '_getch()' from 'getch()' for Visual C++ 2005 Express Edition
	}
}