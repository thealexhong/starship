...
userNumber = 25
offsetHours = 5
affectFileName = "28 Christina Morning-process.csv"
robotFileName = "28_Christina_Flow_2016-02-26_11-59-46.csv"
...
def plotAffect(affectCSVList, robotCSVList, userNumber = 1, interactionType = "Morning"):
    ...
    fig2 = plt.figure(2,figsize=(9,7.1))
    timeStart = times[0]
    timeEnd = times[-1]
    ...
    plt.xticks(np.arange(0, timeEnd-timeStart, (timeEnd-timeStart)/5))
    plt.grid(True)
   ...
   # avgap, = plt.plot(aff_times, AvgA, 'b--', label = "Average Arousal")
    ...
    plt.xticks(np.arange(0, timeEnd-timeStart, (timeEnd-timeStart)/5))
    # rep, = plt.plot(times, REs, 'm-', label = "Robot Emotion")
    hp, = plt.plot(ts, h, '--', label = "Happy", color="darkgreen", linewidth=3)
    ip, = plt.plot(ts, i, '--', label = "Interested", color="#e5c100", linewidth=3)
    sp, = plt.plot(ts, s, '--', label = "Sad", color="cyan", linewidth=3)
    wp, = plt.plot(ts, w, '--', label = "Worried", color="magenta", linewidth=3)
    ap, = plt.plot(ts, a, '--', label = "Angry", color="orangered", linewidth=3)
    ...
    s2p, = plt.plot(ts, s2, '-', label = "Sad 2", color="cyan", linewidth=3)
    w2p, = plt.plot(ts, w2, '-', label = "Worried 2", color="magenta", linewidth=3)
    a2p, = plt.plot(ts, a2, '-', label = "Angry 2", color="orangered", linewidth=3)

    sc1p, = plt.plot(ts, sc1, '-', label = "Scared P", color="#ffa500", linewidth=3)
    sc2p, = plt.plot(ts, sc2, '-', label = "Scared T", color="darkorchid", linewidth=3)
    ...
    fig2.legend((mvp, map,blank,blank,
                 h2p, i2p, s2p, w2p, a2p,sc1p,sc2p,sc3p
                 ),
                ('User Valence', 'User Arousal',"","",
                    "Happy", "Interested", "Sad",  "Worried", "Angry","Scared P (Picked Up)", "Scared T (Touched)", "Scared L (Ledges)"

                 ),
                'lower center', ncol = 3, prop={'size':14})
